...
const PDFExport = (() => {
   ...
   */
  /**
   * Internal Helper: Generate the HTML for the PDF document
   */
  function generateReportHtml({ societyName, gateId, entries, area, dateRange, generatedOn, logoHtml }) {
    ...
    return `
      <div style="font-family: 'Inter', Arial, sans-serif; color: #1a1a2e; background: white; width: 1040px; box-sizing: border-box; margin: 0;">
        <div style="position:relative; padding:15px 40px 10px;">
          <div style="position:absolute; top:50%; left:50%; transform:translate(-50%,-50%) rotate(-35deg); font-size:150px; font-weight:900; color:rgba(220,38,38,0.06); white-space:nowrap; pointer-events:none; z-index:0; user-select:none;">CONFIDENTIAL</div>
          <div style="position:relative; z-index:1;">
            <div style="display:flex; justify-content:space-between; align-items:flex-start; margin-bottom:4px;">
              ...
              <div style="text-align:right; font-size:12px; color:#555;">Society Name: <strong>${escapeHtml(societyName)}</strong></div>
            </div>
            <h1 style="text-align:center; font-size:16px; font-weight:800; color:#1a1a2e; margin:8px 0 4px; letter-spacing:0.5px;">GATIQ – VEHICLE ENTRY AUDIT LOG</h1>
            ...
            <div style="font-size:10px; font-weight:700; color:#dc2626; margin-bottom:8px; text-transform:uppercase;">CONFIDENTIAL - INTERNAL USE ONLY</div>
            ${makeTableHtml(pageChunks[0], config)}
          </div>
        </div>
        ${pageChunks.slice(1).map(chunk => `
          <div style="page-break-before:always; position:relative; padding:20px 40px;">
            <div style="position:absolute; top:50%; left:50%; transform:translate(-50%,-50%) rotate(-35deg); font-size:150px; font-weight:900; color:rgba(220,38,38,0.06); white-space:nowrap; pointer-events:none; z-index:0;">CONFIDENTIAL</div>
            <div style="position:relative; z-index:1;">${makeTableHtml(chunk, config)}</div>
          </div>
        `).join('')}
      </div>`;
  }

  const makeTableHtml = (chunk, config) => {
    ...
    if (chunk.length > 0) {
      ...
      tbody = `<tr>
          <td colspan="${config.columns.length}" style="${tdStyle} text-align:center; color:#999; padding:30px;">No entries recorded</td>
        </tr>`;
    }
  ...
  async function exportPDF({ societyName, gateId, entries, area }) {
    ...
    if (typeof PDF_LOGO_BASE64 !== 'undefined' && PDF_LOGO_BASE64) logoHtml = `<img src="${PDF_LOGO_BASE64}" alt="GATIQ Logo" style="height:36px; width:auto; object-fit:contain; vertical-align:middle;">`;

    const htmlContent = generateReportHtml({ societyName, gateId, entries, area, dateRange, generatedOn, logoHtml });
    ...
    return html2pdf().set(options).from(htmlContent).toPdf().get('pdf').then((pdf) => {
  ...
  /**
   * Internal version that returns base64 instead of downloading
   */
  async function exportPDFBase64({ societyName, gateId, entries, area }) {
    const now = new Date();
    const generatedOn = formatFullDate(now);
    const dateRange = { 
        from: entries.length ? entries[entries.length-1].date : '-', 
        to: entries.length ? entries[0].date : '-' 
    };
    let logoHtml = `<div style="width:36px; height:36px; background:#4f46e5; border-radius:6px; display:inline-block; vertical-align:middle; text-align:center; color:white; font-weight:bold; font-size:20px; line-height:36px; font-family:sans-serif;">G</div>`;
    if (typeof PDF_LOGO_BASE64 !== 'undefined' && PDF_LOGO_BASE64) logoHtml = `<img src="${PDF_LOGO_BASE64}" alt="GATIQ Logo" style="height:36px; width:auto; object-fit:contain; vertical-align:middle;">`;
    ...
    return html2pdf().set(options).from(htmlContent).toPdf().output('blob').then(async (blob) => {
  ...
  return { exportPDF, exportPDFBase64 };
})();
